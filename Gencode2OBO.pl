...
use lib "$FindBin::Bin/schema+agents/lib";
use TabParser;
use BP::Model;
...
if(scalar(@ARGV)==3) {
	...
	if(defined($class3File)) {
		if($class3File =~ /\.gz$/) {
			$openMode = '-|';
			@command = (BP::Loader::Agents::GUNZIP,'-c',$class3File);
		} else {
			$openMode = '<';
			@command = ($class3File);
		}
		...
		if($GencodeGfile =~ /\.gz$/) {
			$openMode = '-|';
			@command = (BP::Loader::Agents::GUNZIP,'-c',$GencodeGfile);
		} else {
			$openMode = '<';
		...
		if($GencodeTfile =~ /\.gz$/) {
			$openMode = '-|';
			@command = (BP::Loader::Agents::GUNZIP,'-c',$GencodeTfile);
		} else {
			$openMode = '<';
